...
@section('title', 'Produk | Jager Bakery')
                    ...
                    <div class="wrapper d-flex flex-column gap-3 w-100">
                        ...
                        <div class="category-list d-flex flex-column gap-2">
                            <a href="{{ route('frontend.product.index') }}" class="d-flex w-100 align-items-center justify-content-between {{ request()->category ? '' : 'active' }}">
                                <span>All</span>
                                <span>({{ $categories->sum(fn($category) => $category->products->count()) }})</span>
                            </a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchInput.addEventListener('input', function() {
            ...
            searchInput.dataset.timer = setTimeout(function() {
                searchForm.submit();
            }, 300); // Mengirim form setelah 300ms tidak ada input baru
        });
    });
</script>
@endsection